...
let append_to_file filename str =
  Out_channel.with_open_gen
    [ Open_text; Open_append; Open_creat ]
    0o644 filename
  @@ fun oc -> Out_channel.output_string oc str
...
let box_upload_file env token_file id path filename =
  ...
  let token =
    read_from_file token_file |> String.trim |> String.cat "Bearer "
  in
  ...
  if Http.Status.compare resp.status `Created = 0 then
    ...
    Fmt.epr "Upload file %s returned unexpected HTTP status: %a" filename
      Http.Status.pp resp.status;
    Error resp.status)

type part = { offset : int; part_id : string; sha1 : string; size : int }
...
let box_create_upload_session sw2 env token_file id src filename size =
  ...
  if Http.Status.compare resp.status `Created = 0 then
    ...
    Fmt.epr "Create upload session for %s returned unexpected HTTP status: %a"
      filename Http.Status.pp resp.status;
    Error resp.status)
...
let box_create_folder env token_file id folder =
  ...
  if Http.Status.compare resp.status `Created = 0 then
    ...
    let reply = Yojson.Safe.from_string reply in
    let newid =
      Yojson.Safe.Util.member "id" reply |> Yojson.Safe.Util.to_string
    in
    ...
    Fmt.epr "Create folder %s returned unexpected HTTP status: %a" folder
      Http.Status.pp resp.status;
    Error resp.status)
...
let box_upload_part_file env token_file session =
  ...
  let headers = Http.Header.add headers "authorization" token in
  let headers =
    Http.Header.add headers "content-type" "application/octet-stream"
  in
  let block, _, len = session.next () |> Option.value ~default:("", 0, 0) in
  let range =
    Printf.sprintf "bytes %i-%i/%i" session.index
      (session.index + len - 1)
      session.size
  in
  let headers = Http.Header.add headers "content-range" range in
  let sha1 =
    Sha1.string block |> Sha1.to_bin |> Base64.encode_string
    |> String.cat "sha="
  in
  ...
  let resp, body =
    Client.put ~body:(Body.of_string block) ~sw ~headers client
      session.upload_part
  in
  if Http.Status.compare resp.status `OK = 0 then
    let reply = Eio.Buf_read.(parse_exn take_all) body ~max_size:max_int in
    ...
    let offset =
      Yojson.Safe.Util.member "offset" part |> Yojson.Safe.Util.to_int
    in
    let part_id =
      Yojson.Safe.Util.member "part_id" part |> Yojson.Safe.Util.to_string
    in
    let sha1 =
      Yojson.Safe.Util.member "sha1" part |> Yojson.Safe.Util.to_string
    in
    let size = Yojson.Safe.Util.member "size" part |> Yojson.Safe.Util.to_int in
    Ok
      {
        session with
        index = session.index + len;
        part_list = { offset; part_id; sha1; size } :: session.part_list;
      }
  else (
    Fmt.epr "Upload part returned unexpected HTTP status: %a" Http.Status.pp
      resp.status;
    Error resp.status)
...
let box_upload_commit env token_file session =
  ...
  let headers = Http.Header.add headers "content-type" "application/json" in
  let sha1 =
    Sha1.finalize session.ctx |> Sha1.to_bin |> Base64.encode_string
    |> String.cat "sha="
  in
  let headers = Http.Header.add headers "digest" sha1 in
  let json_string =
  ...
  let resp, body =
    Client.post
      ~body:(Body.of_string json_string)
      ~sw ~headers client session.commit
  in
  if Http.Status.compare resp.status `Created = 0 then
    let reply = Eio.Buf_read.(parse_exn take_all) body ~max_size:max_int in
    let reply = Yojson.Safe.from_string reply in
    ...
    Fmt.epr "Upload commit returned unexpected HTTP status: %a" Http.Status.pp
      resp.status;
    Error resp.status)
...
let box_delete_file env token_file id =
  ...
  if Http.Status.compare resp.status `No_content = 0 then Ok ()
    ...
    Fmt.epr "Delete file %i returned unexpected HTTP status: %a" id
      Http.Status.pp resp.status;
    Error resp.status)
...
let upload env token_file id src filename =
  ...
  let session =
    box_create_upload_session sw env token_file id src filename stat.st_size
    |> Result.get_ok
  in
  let rec loop session =
    let () = Eio.traceln "%i/%i" session.index session.size in
    let session =
      box_upload_part_file env token_file session |> Result.get_ok
    in
    ...
    | hd :: tl when Sys.is_directory (src / hd) ->
        ...
        let () =
          if Option.is_none v then
            ...
            let id =
              box_check env token_file 0 (String.split_on_char '/' (dst / dir))
            in
            ignore
              (box_create_folder env token_file
                 (Option.value ~default:0 id)
                 name)
        in
        let d =
          Sys.readdir (src / hd)
...
let rec rev_scan env token_file src dst fldr path id =
  ...
  | None ->
      ...
      if String.starts_with ~prefix:(dst ^ "/") path then
        let src =
          ...
          / String.sub path
              (1 + String.length dst)
              (String.length path - String.length dst - 1)
        in
  ...
  | Some map ->
      Entries.iter
        (fun s id -> rev_scan env token_file src dst fldr (path / s) id)
        map
